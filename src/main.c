...
/*   Updated: 2024/05/28 06:38:42 by laubry           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "philo.h"
...
int	init_philo(t_data *data, t_philo *philo)
{
	...
	while (i < data->nbr_philo)
	{
		...
		if (pthread_mutex_init(&philo[i].fork_left, NULL) != 0)
			return(print_error(ERROR_CREATE_MUTEX, philo));
		if (i > 0)
...
int	make_philo(t_data data, t_philo *philo)
{
	...
	while (i < data.nbr_philo)
	{
		if (pthread_create(&philo[i].tid, NULL, routine, &philo[i]) != 0)
			return (print_error(ERROR_CREATE_PHILO, philo));
		i++;
	}
	return (1);
}
...
int	main(int argc, char **argv)
{
	...
	if (!philo)
		return (print_error(ERROR_MALLOC, philo));
